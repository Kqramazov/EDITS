...
def load_bail(dataset, sens_attr="WHITE", predict_attr="RECID", path="dataset/bail/", label_number=1000, seed=10):
    ...
    if os.path.exists(f'{path}/{dataset}_edges.txt'):
        ...
        edges_unordered = build_relationship(idx_features_labels[header], thresh=0.6,seed=seed)
        np.savetxt(f'{path}/{dataset}_edges.txt', edges_unordered)
...
#
    ...
    if os.path.exists(f'{path}/{dataset}_edges.txt'):
        ...
        edges_unordered = build_relationship(idx_features_labels[header], thresh=0.8,seed=seed)
        np.savetxt(f'{path}/{dataset}_edges.txt', edges_unordered)
